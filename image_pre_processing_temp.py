...
for i, data in enumerate(celeba_dataloader, 1):
    ...
    if faces:
        ...
        # 在眼睛位置画白色矩形
        cv2.circle(img_np_cp, (int(left_eye[0]), int(left_eye[1])), 30, (255, 255, 255), -1)
        # cv2.rectangle(img_np_cp, (int(left_eye[0])-5, int(left_eye[1])-5), (int(left_eye[0])+5, int(left_eye[1])+5), (255, 255, 255), -1)
        # cv2.rectangle(img_np_cp, (int(right_eye[0])-5, int(right_eye[1])-5), (int(right_eye[0])+5, int(right_eye[1])+5), (255, 255, 255), -1)
        # cv2.rectangle(img_np_cp, (int(nose[0])-5, int(nose[1])-5), (int(nose[0])+5, int(nose[1])+5), (255, 255, 255), -1)
        # cv2.rectangle(img_np_cp, (int(mouth_left[0])-5, int(mouth_left[1])-5), (int(mouth_left[0])+5, int(mouth_left[1])+5), (255, 255, 255), -1)
        # cv2.rectangle(img_np_cp, (int(mouth_right[0])-5, int(mouth_right[1])-5), (int(mouth_right[0])+5, int(mouth_right[1])+5), (255, 255, 255), -1)
